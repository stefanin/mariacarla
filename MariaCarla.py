...
"""
            ...
            if "NON POSSO GENERARE LA QUERY" in generated_sql.upper() or not generated_sql.upper().startswith("SELECT"):
                ...
                if "error" in query_results:
                    ...
                    for i, row in enumerate(query_results['rows'][:10]):
                        results_for_llm += str(row) + "\n"
                    if len(query_results['rows']) > 10:
                        results_for_llm += f"... e altre {len(query_results['rows']) - 10} righe.\n"
                    
                    system_report_gen = "Sei un assistente che spiega i risultati di una query SQL in modo chiaro e conciso in italiano."
                    prompt_report_gen = f"""
                    Basandoti sui seguenti risultati della query, fornisci un report in formato tabella eliminando i caratteri non necessari, 
                    se nel report vuoi restituire una tabella, formattala come una tabella html e aggingi nella risposta il tag <TABELLAHTML>,
                    massimo 100 righe o una risposta alla domanda originale dell'utente ('{user_question}').\n\nDati:\n{results_for_llm}\n\nReport:"""
